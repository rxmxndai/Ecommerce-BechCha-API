...
const verifyOTP = tryCatch(async (req, res, next) => {
  ...
  if (expiresAt < Date.now()) {
    ...
    return next("OTP timeout. Please request for new one.", undefined);
  }
  ...
  if (!validOTP) {
    return next("OTP verification failed", undefined);
  }
...
const loginUser = tryCatch(async (req, res) => {
  ...
  if (!user.isVerified)
    throw new customError("Please verify OTP through email", 400);

  // delete old jwt from db too
  const newTokenArray = !cookies?.jwt
    ? user.refreshToken
